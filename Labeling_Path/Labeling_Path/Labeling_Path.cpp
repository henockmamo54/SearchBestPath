	...
	int Nrow, int Ncol, int NEH, float fBWE)
{
	...
	for (i = 0; i < Nrow; i++)
	{
		...
		for (j = 0; j < Ncol; j++)
		{
			...
			temp_path = temp_path + index_X[i, j].ToString() + "," + index_Y[i, j].ToString() + " ";
...
List<String^>^ Label_Path::Labeling_Path_Fractional_Synthesis2(array <float, 2>^ Fractional_Synthesis_Rate,
	int Nrow, int Ncol, int NEH, float fBWE)
{
	int i, j, i1, j1, i0, j0, iMax;
		...
		//for (i = 0; i < 1; i++)
	{
		for (j = 1; j < Ncol; j++)
		{
			...
			{
				float min_diff = (float)1E6;
				for (i0 = 0; i0 < Nrow; i0++)
				{
					float temp = Math::Abs((fBest_Fractional_Synthesis_Rate_Time[i, j - 1] - Fractional_Synthesis_Rate[i0, j]));
					if (temp < min_diff) {

						min_diff = Math::Abs((fBest_Fractional_Synthesis_Rate_Time[i, j - 1] - Fractional_Synthesis_Rate[i0, j]));
